...
## Hero slide ↔ product-card mismatch (2026-09-20, src/pages/Home.tsx — deployed & live-verified)
- **Bug found (live prod, 3 browser contexts):** every hero slide rendered the SAME 3 cards — the global first-3 in-stock products. Prod API order puts the 7 MCdodo cables first, so the holders slide («هولدرهای مگنتی خودرو و پایه‌های رومیزی ضدلغزش», CTA `/products?category=هولدر و نگهدارنده`) displayed 3 cable cards (ids 5639/5638/5637, category «کابل و سیم») on chromium desktop 1366×900, chromium mobile 390×844 and webkit desktop.
- **Root cause:** `Home.tsx` `heroProducts` memo = `products.filter(stock>0).slice(0,3)` — catalogue-global, passed unchanged to every `HeroSlideContent` (desktop stacked grid + mobile single slide).
- **Fix (code, smallest):** `heroProductsBySlide` memo maps each slide id → products whose `category` equals the slide CTA's `?category=` param (same regex + `decodeURIComponent` the empty-cat guard uses); falls back to in-stock first-3 only when the slide link carries no category or the category has no in-stock rows. `heroProducts` now derived from the active slide id.
- **Verified:** gate `npm run verify` EXIT=0 (62 files / 482 tests, build OK, SEC-01/SEC-03 PASS); dev-DB dual-engine probe (chromium desktop+mobile, webkit desktop) → holders slide cards 100% «هولدر و نگهدارنده», cable slide cards 3/3 «کابل و سیم», 0 console errors.
- **Pre-existing, NOT this change:** `e2e/real-user.spec.ts:122` fails in its own `beforeEach` warm-up hook (browser closed / 30s timeout) with the patch stashed as well — harness/env issue, unrelated to the hero fix.
