...
- ⚠️ **باز:** `features`/مشخصات فنی خالی (سند Google Docs خصوصی است — ورودی لازم)؛ کیفیت ۹ مدل زیر ۱۰۰۰px واقعی + دو عکس (`eh127`, `eh186`) پس‌زمینهٔ غیرسفید؛ تأیید بصری vision انجام نشد (سرویس ۵۰۳) — کانتکت‌شیت ۳۷ عکس برای بازبینی: `.hermes/imports` → `/tmp/earldom-sheet-v2.jpg`.
