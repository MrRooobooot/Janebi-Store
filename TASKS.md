...
- ⚠️ **باز:** `features`/مشخصات فنی خالی (سند Google Docs خصوصی است — ورودی لازم)؛ کیفیت ۹ مدل زیر ۱۰۰۰px واقعی + دو عکس (`eh127`, `eh186`) پس‌زمینهٔ غیرسفید؛ تأیید بصری vision انجام نشد (سرویس ۵۰۳) — کانتکت‌شیت قبلی (`/tmp/earldom-sheet-v2.jpg`) با پاک شدن `/tmp` از بین رفت — برای بازبینی باید بازتولید شود.
