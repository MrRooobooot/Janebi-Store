...
## هرو: کارت‌های واقعی محصول (کامنت درون‌برنامه‌ای، ۱۴۰۵/۰۶/۲۵ — live)
| # | کامنت کاربر | ریشه | فیکس | اثبات |
|---|-------------|------|------|-------|
| ۱ | «این بخش رو بازطراحی کن به شکلی که کارت‌های واقعی رو نشون بده» (هیرو، اسلاید کابل/محافظ) | ستون تصویر هیرو یک تایل تزئینی SVG تک بود (آیکون کوچک، عدم تراز، فضای خالی) و کپی اسلایدها به دسته‌های **بی‌موجودی** اشاره می‌کرد | `HeroSlideContent` اکنون **۳ کارت واقعی** از API (عکس + عنوان + قیمت + لینک PDP) + تایل «مشاهده همه» رندر می‌کند؛ `Home` فقط اسلایدهایی را نگه می‌دارد که دستهٔ CTAشان موجودی دارد؛ تایل SVG تنها fallback در حالت کاتالوگ خالی؛ `heroSlide1Link` پیش‌فرض از `هولدر و پایه` (دستهٔ ناموجود) به `هولدر و نگهدارنده` اصلاح شد | پروب `hero-cards-0916.mjs` روی prod (Chromium+WebKit، ۱۲۸۰ لایت/دارک و ۳۹۰): ۳ کارت واقعی `ear-*.webp`، `broken: 0`، `svgTile: 0`، لینک‌ها `/product/5632|5631|5630` |
| ۲ | (بازبینی vision) ناهم‌ترازی قیمت‌ها، برچسب‌های رهاشده، فلش‌های اسلایدر روی لبه | کارت‌ها هم‌ارتفاع نبودند؛ برچسب‌ها بدون چیپ؛ ناوبری اسلایدر با یک اسلاید زنده بی‌معنا | `h-full` + `mt-auto` روی قیمت؛ چیپ «موجود در انبار» (نقطهٔ سبز) و چیپ «N کالا»؛ فلش‌ها و نقطه‌های اسلایدر فقط با `heroSlides.length > 1` رندر می‌شوند؛ خط‌شکن کد مدل با نیم‌فاصلهٔ سخت (`U+2011`) | بررسی vision روی اسکرین‌شات prod: قیمت‌ها هم‌تراز، چیپ‌ها ساختارمند، ناوبری حذف‌شده |

## پاس موبایل — «حس اپلیکیشن باکیفیت» (۱۴۰۵/۰۶/۲۵، live)
**روش:** اسکیل `ui-ux-pro-max` (design-system + دامنهٔ ux + استک html-tailwind) روی شواهد اندازه‌گیری‌شدهٔ prod (`scripts/probes/mobile-ux-audit-0916.mjs`, ۳۹۰×۸۴۴، Chromium+WebKit) — نه حدس.

| # | ریشهٔ اندازه‌گیری‌شده | فیکس | شاهد قبل → بعد |
|---|----------------------|------|----------------|
| ۱ | هدر چسبان ۱۵۸px روی موبایل (نوار اعلان + ناوبری + جستجو) | نوار اعلان `py-1`، ردیف ناوبری `h-14`، و **جستجو از داخل هدر چسبان بیرون آمد** (زیر هدر، با محتوا اسکرول می‌شود) | `header 158px → 98px` (WebKit 160→98) |
| ۲ | ۲۷ هدف لمسی زیر ۴۴px در خانه (۵۳ در لیست محصولات، ۲۰ در PDP) | چیپ‌های تب `min-h-11`، دکمهٔ فیلتر/`select` مرتب‌سازی `min-h-11`، تب‌های PDP `min-h-11`، صفحه‌بندی `min-h-11` + `w-11 h-11`، بردکرامب `min-h-11`، لینک سریع فوتر و ارسال خبرنامه `min-h-11` | `home 27→3` · `products 53→3` · `pdp 20→4` (WebKit: ۱ و ۱ و ۲) |
| ۳ | کارت محصول موبایل شلوغ (برند + دسته + امتیاز + گارانتی + مقایسه + بج کم‌موجودی) | حذف اقلام غیرضروری از نمای موبایل (`hidden sm:*`)، دکمهٔ مقایسه فقط دسکتاپ، علاقه‌مندی ۴۴px، بج کم‌موجودی فقط برای «۱ عدد باقی‌مانده» (روی هر ۳۷ کالا ۲ عدد بود و بج تکراری حس فیک می‌داد)، تایتل/قیمت/تومان درشت‌تر و تراز | بازبینی vision روی اسکرین‌شات prod: کارت‌ها هم‌ارتفاع، قیمت/CTA جدا و خوانا |
| ۴ | سرریز افقی: نوارهای اسکرولی با `-mx-4 px-4` از لبه بیرون می‌زدند (offender تا ۳۹۴px>۳۹۰) و کاروسل دسته‌بندی با ۱ دسته فضای خالی می‌ساخت | حذف ترفند مارجین منفی؛ با کمتر از ۳ دسته، کارت تمام‌عرض (بدون کاروسل بی‌معنا) | `horizontalOverflow = 0` در هر سه صفحه، دو موتور |
| ۵ | طول صفحهٔ خانه ۶۰۰۱px (حس بی‌پایان) + تکرار دو بار بلوک مزایا (میانه + فوتر) | بلوک مزایای میانه در موبایل مخفی (نسخهٔ فشردهٔ فوتر می‌ماند) | `scrollHeight 6001 → 5525` (−۸٪) |
| ۶ | نوار پایین ۷۴px بدون احتساب safe-area | `pb-[max(0.375rem,env(safe-area-inset-bottom))]` + برچسب ۱۱px | `navH` ۷۴ با safe-area پویا (iOS home-indicator دیگر روی تب‌ها نمی‌افتد) |
