...
| 0914 | برچسب روش پرداخت در DB سخت‌کد `زرین‌پال` است (`server/routes/orders.ts:217`) | — | — | اگر circuit breaker زرین‌پال را باز کند و **سامان** پرداخت را انجام دهد، برچسب ذخیره‌شده اشتباه می‌شود | خواندن کد | `paymentMethod === "online" ? "پرداخت آنلاین زرین‌پال" : "پرداخت در محل"` | ذخیرهٔ provider برگشتی از `/api/payment/request` (یا برچسب بی‌طرف «پرداخت آنلاین») | OPEN |
